...
$term = mysql_real_escape_string($_REQUEST['term']);}
				...
				$result = $db->prepare("SELECT * FROM collection WHERE name LIKE '%".$term."%'  ORDER BY `date2` ASC");?>
Payments from&nbsp;<?php echo $_GET['d1'] ?>&nbsp;to&nbsp;<?php echo $_GET['d2'] ?> Made by <?php echo $_GET['term'] ?>
</div>
	...
	<thead>
		<tr>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$results = $db->prepare("SELECT sum(amount2) FROM collection WHERE date2 BETWEEN :a AND :b ORDER BY `date2`");
