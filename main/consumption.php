...
<div class="container-fluid">
	...
	<div class="span2">
          <div class="well sidebar-nav">
			...
			<li class="active"><a href="salesreport.php?d1=0&d2=0"><i class="icon-bar-chart icon-2x"></i> Consumption Report</a>                </li>
...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
		<script type="text/javascript">getElementById("showtime");</script>
		
			<?php
				include('../connect.php');
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$result = $db->prepare("SELECT * FROM sales_order WHERE date BETWEEN :a-1 AND :b ORDER by transaction_id DESC ");
	...
	<thead>
		<tr>
			...
			<?php
				...
				$d1=$_GET['d1'];
				$d2=$_GET['d2'];
				$results = $db->prepare("SELECT sum(qty) FROM sales_order WHERE date BETWEEN :a AND :b ORDER by transaction_id DESC");
				...
				<th colspan="1" style="border-top:1px solid #999999">
			<?php 
				$resultia = $db->prepare("SELECT sum(qty) FROM sales_order WHERE date BETWEEN :c AND :d ORDER by gen_name DESC");
