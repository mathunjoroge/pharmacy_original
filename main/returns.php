...
<option></option>
	<?php
	include('../connect.php');
	$result = $db->prepare("SELECT * FROM products WHERE `active`=1");
		$result->execute();
...
<table class="table table-bordered" id="resultTable" data-responsive="table">
	...
	<tbody>
			...
			<?php
				$id=$_GET['invoice'];
				include('../connect.php');
				$result = $db->prepare("SELECT transaction_id,product_code,gen_name,product,product_name,o_price,amount,returns.qty AS qty,product_id,batch  FROM returns RIGHT OUTER JOIN products ON products.product_id=returns.product WHERE invoice= :userid");
			...
			<td ><a rel="facebox" href="editreturn.php?id=<?php echo $row['transaction_id']; ?>&btid=<?php echo $row['transaction_id']; ?>"><button class="btn btn-mini btn-warning"><i class="icon icon-remove"></i> edit </button></a>
				
			<a href="deletereturn.php?id=<?php echo $row['transaction_id']; ?>&btid=<?php echo $row['batch']; ?>&qty=<?php echo $row['qty']; ?>&product_id=<?php echo $row['product_id']; ?>&invoice=<?php echo $id; ?>"><button class="delbutton" onclick="return confirm('Are you sure you want to Remove?'); ><i class="icon icon-remove"></i>delete</button></a>
			</tr>
			<?php
				}
			?>
...
<a rel="facebox" href="checkreturn.php?pt=<?php echo $_GET['id']?>&invoice=<?php echo $_GET['invoice']?>&total=<?php echo $fgfg ?>&totalprof=<?php echo '' ?>&cashier=<?php echo $_SESSION['SESS_FIRST_NAME']?>"><button class="btn btn-success btn-large btn-block" ><i class="icon icon-save icon-large" accesskey="s"></i> SAVE</button></a>
