...
$term = mysql_real_escape_string($_REQUEST['term']);}
				$result = $db->prepare("SELECT * FROM collection WHERE  date BETWEEN :a AND :b AND name LIKE '%".$term."%'  ORDER BY `name` ASC");
				$result->bindParam(':a', $d1);
				$result->bindParam(':b', $d2);				
				$result->bindParam(':d', $d);			
				for($i=0; $row = $result->fetch(); $i++)
			?>
			<tr class="record"><tbody>
	...
	<thead>
		<tr>
			...
			<?php
				...
				$results = $db->prepare("SELECT sum(amount) FROM collection WHERE date BETWEEN :a AND :b AND name LIKE '%".$term."%' ORDER BY `name` ASC");
