...
<input type="text" name="filter" style="padding:15px;" id="filter" placeholder="Search product..." autocomplete="off" />
				...
				if(isset($_GET['id']))
{
	...
	$id=1;
				...
				$result = $db->prepare("SELECT* FROM products RIGHT OUTER JOIN batch ON batch.product_id=products.product_id ORDER BY expirydate  ASC, product_code DESC");
				$result->execute();
