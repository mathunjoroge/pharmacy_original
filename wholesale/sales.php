...
<a  href="../main/index.php"><button class="btn btn-success btn-large" style="float: none;" ><i class="icon icon-circle-arrow-left icon-large"></i> Back</button></a>&nbsp;<a  href="../main/returns.php?id=cash&invoice=<?php echo $finalcode; ?>""><button class="btn btn-success btn-large" style="float: none;" >sales return</button></a>
...
?>
    ...
    <option value="<?php echo $row['product_id']; ?>" data-qty="<?=$row['qty'];?>" data-pr="<?=$row['wholesaleprice'];?>"  data-minpr="<?=$row['maxdiscre']; ?>" data-bpr="<?=$row['o_price']; ?>" data-maxdiscpc="<?=$row['maxdiscpr'];?>">
        <?=$row['gen_name'];?> -
            <?=$row['product_code'];?>
    </option>
<?php endfor;?>
...
<td><input type="number" id="pricemin" name="pr" min="" placeholder="price" id="fpr"  style="width: 68px; height:30px; padding-top:6px; padding-bottom: 4px; margin-right: 4px; font-size:15px;"></td>
<input type="hidden" name="date"  value="<?php $date = date('Y-m-d');
                $d11 = strtotime ( $date ) ;
                $d11 = date ('Y-m-d' , $d11);
                echo $d11; ?>" />
<td><input type="number" name="pc" max="" placeholder="disc" id="disc" style="width: 68px; height:30px; padding-top:6px; padding-bottom: 4px; margin-right: 4px; font-size:15px;" /></td>
...
$id = $_GET['invoice'];
include '../connect.php';
$result = $db->prepare("SELECT *  FROM sales_order  RIGHT OUTER JOIN products ON products.product_id=sales_order.product WHERE invoice= :userid AND `rest`=0 ");
...
if ($row['has_bonus']==1) {
	...
	# code...
?>
			<a href="delete.php?trans_id=<?php echo $row['transaction_id']; ?>&invoice=<?php echo $_GET['invoice']; ?>&qty=<?php echo $row['quantity']; ?>&prod_id=<?php echo $row['product']; ?>"><button class="btn btn-mini btn-warning"><i class="icon icon-remove"></i> delete </button></a><?php } ?>
